...
class BDD100K(data.Dataset):
  num_classes = 10
  default_resolution = [720, 1280]   # TODO h,w or w,h
  ...
  def __init__(self, opt, split):
    ...
    self.data_dir = os.path.join(opt.data_dir, 'bdd100k') # TODO make sys link
    ...
    if split == 'test':
      ...
      if opt.task == 'exdet':
        ...
        self.annot_path = os.path.join(
          self.data_dir, 'annotations', # TODO make sys link
          'bdd100k_labels_images_det_coco_{}.json').format(split)
    self.max_objs = 128 #TODO to check
    self.class_name = [
